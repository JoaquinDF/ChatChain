	...
	public void run() {
		...
		try {
			...
			while (true) {
				...
				String newBC = new String(recv.getData(), 0, recv.getLength(), "UTF-8");
				...
				if ((!ChatBlock.HashString(gson.toJson(prevBlock)).equals(newBlock.getPrevBlockHash()))
						|| !newBlock.getTextHash().equals(ChatBlock.HashString(newBlock.getText()))) {
					...
					DatagramPacket packet = new DatagramPacket(msg.getBytes(), msg.length(), unicastanswer,
							ChatChain.SINGLECASTPORT);
					...
					target.path("ChatChain").path("add").queryParam("justadd", "true")
							.queryParam("text", URLEncoder.encode(newBC, "UTF-8")).request(MediaType.TEXT_PLAIN)
							.get(String.class);
